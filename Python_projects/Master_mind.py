...
def MasterMind(randomNumberList,guessed,num_of_guess):
    ...
    while(num_of_guess>0 and (isCorrectGuessed ==False)):
        ...
        if numofdigits(guessed)!=len(randomNumberList):#number of digits are not same as input by the user earlier
            print(f"You are playing with a 4 digit number but have entered a number with {numofdigits} digits")
        else:
...
if MasterMind(randomNumberList,guessed,num_of_guess): #if user guessed the number          
